<!DOCTYPE html>
<html>
<head>
    <title>Hi</title>
    <style>
        body {
            font-size:10px;
            margin:0px;
            line-height: 8px;
        }
        @page {
            margin:0px;
        }
        .page-break {
            page-break-after: always;
        }
        .aturan {
            font-size:15px;
            line-height: 10px;
            /* font-size:10px; */
            font-weight:bold;
        }
        .kanan {
            /* width: 60px; */
            /* text-align:left; */
            /* font-size:10px; */
        }
        tr.border_bottom td {
            border-bottom:1pt solid black;
        }
    </style>
</head>

<body>
    @foreach($label as $i => $label)
        <!-- do something here | divs -->
        <table border="0" cellpadding="1" cellspacing="0" align="center" style="margin:0;padding:0; width:242px;">
            <tr style="line-height:8px;" class="border_bottom">
                <td style="text-align:center; width:50px;padding-bottom:3px;padding-top:3px;">
                    <img src="data:image/png;base64," style="height:35px;">
                </td>
                <td style="text-align:center;padding-bottom:3px;" colspan="4" style="width:70px;">
                    <h2 style="text-align:center;margin:0;font-size:9px;">INSTALASI FARMASI RUMAH SAKIT <br /> UNIVERSITAS TANJUNGPURA</h2>
                    <span style="text-align:center;font-size:7px;">Jl. Prof.Dr.Hadari Nawawi Pontianak 78124
                    Telp. 765324 <br/>
                    Dede Robianto, S.Far., Apt. <br/>
                    No. SIPA : 19880711/SIPA_6171/2013/2054</span>
                </td>
                {{-- <td></td> --}}
            </tr>
            {{-- <tr>
                <td style="" colspan="5">
                    <hr />
                </td>
            </tr> --}}
            <tr>
                <td colspan="2" style="padding-top:3px;width:70px;">
                    No RM
                </td>
                <td style="padding-top:3px;width:75px;">
                    : {{$label->mr}}
                </td>
                <td class="kanan" colspan="2" style="padding-top:3px;">Tgl : {{$today}}</td>
            </tr>
            <tr>
                <td colspan="2">
                    Nama Pasien
                </td>
                <td colspan="3">
                    : {{$label->nama}}
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    Nama Obat
                </td>
                <td colspan="3">
                    : {{$label->obat}}
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    Indikasi Obat
                </td>
                <td colspan="3">
                    : {{$label->indikasi}}
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    Rute Pemberian
                </td>
                <td colspan="3">
                : {{$label->rute_pemberian}}
                </td>
            </tr>
            <tr>
                <td class="aturan" colspan="3" style="padding-top:10px;">
                    Sehari {{$label->x_sehari}} X
                </td>
                <td colspan="2" style="padding-top:10px;">
                    ED : {{$label->ed}}
                </td>
            </tr>
            <tr>
                <td class="aturan" colspan="3">
                    {{$label->per_jam}}
                </td>
                <td colspan="2">
                    Jumlah : {{$label->jumlah}} {{$label->jumlah_jenis}}
                </td>
            </tr>
            <tr>
                <td class="aturan" colspan="5">
                    @if( !empty($label->note_x))
                        {{$label->note_x}} X {{$label->note}}
                    @endif
                </td>
                {{-- <td colspan="2">
                    ED : {{$label->ed}}
                </td> --}}
            </tr>
            {{-- <tr>
                <td class="aturan" colspan="5">
                    {{$label->note}}
                </td> --}}
                {{-- <td colspan="2">
                    Jumlah : {{$label->jumlah}} {{$label->jumlah_jenis}}
                </td> --}}
            {{-- </tr> --}}
            <tr>
                <td class="aturan" colspan="5" style="text-align:center;padding-top:10px;line-height:12px;">
                    <i>OBAT LUAR</i><br /><i>SEMOGA LEKAS SEMBUH</i>
                </td>
            </tr>
        </table>

        @if($i+1 < $count)
            <div class="page-break"></div>
        @endif
    @endforeach


</body>
</html>
